// Package gpuexportercmd is the body of gpu-exporter — root fdinfo→/run bridge for Intel xe-driver GPUs.
//
...
// Run is the exporter loop. The binary has never taken flags; the empty
// FlagSet exists so --help answers like the other commands instead of
// starting the loop, and so a stray argument is rejected rather than ignored.
// Errors are returned with the "gpu-exporter: " prefix the binary always
// printed, so the wrapper writing err to stderr reproduces the old output.
//
// The loop ends only when ctx is cancelled: the standalone binary passes a
// background context and so runs until killed, exactly as before.
func Run(ctx context.Context, args []string) error {
	fs := flag.NewFlagSet("gpu-exporter", flag.ContinueOnError)
	if err := fs.Parse(args); err != nil {
		return exitcode.Parse(err)
	}
	if err := os.MkdirAll(outDir, 0o755); err != nil {
		return fmt.Errorf("gpu-exporter: %w", err)
	}
	cards := discoverXeCards()
	if len(cards) == 0 {
		return errors.New("gpu-exporter: no Intel xe cards found under /sys/class/drm")
	}
