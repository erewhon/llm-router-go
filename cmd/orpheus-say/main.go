...
//
// The body lives in package cli (cli.Say) so the unified pitf CLI can
...
func main() {
	cli.Version = version
	// A background context on purpose: cli.Say installs its own signal
	// handling at the point the old main did, so startup and shutdown
	// behave exactly as before.
	err := cli.Say(context.Background(), os.Args[1:])
