// Command tool-proxy is the Go rewrite of the Python tool proxy: chat
// completions with the tool-execution loop and the auto-router.
//
// The body lives in package cli (cli.ToolProxy) so the unified pitf CLI can
...
func main() {
	cli.Version = version
	// A background context on purpose: cli.ToolProxy installs its own signal
	// handling at the point the old main did, so startup and shutdown
	// behave exactly as before.
	err := cli.ToolProxy(context.Background(), os.Args[1:])
